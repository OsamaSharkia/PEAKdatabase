...
if (typeof window !== "undefined") {
  if (window.supabase) {
    ...
    // Create a fallback client if the script is loaded after this file
    ...
    try {
      ...
      if (session) {
        ...
        if (!profileError && profile) {
          ...
          const { error: insertError } = await window.supabaseClient.from("user_profiles").insert(newProfile)
  ...
  async login(email, password) {
    try {
      ...
      if (profileError && profileError.code === "PGRST116") {
        ...
        await window.supabaseClient.from("user_profiles").insert(newProfile)
      }
  ...
  async register(email, password, name) {
    try {
      ...
      if (data.user) {
        ...
        const { error: profileError } = await window.supabaseClient.from("user_profiles").insert(newProfile)
      ...
      try {
        ...
        if (error) {
          ...
          if (error.code === "PGRST116") {
            console.log("Profile not found in getUserProfile, creating one")
    ...
    async updateUserProfile(profileData) {
      try {
        ...
        if (checkError && checkError.code === "PGRST116") {
          ...
          const { error: insertError } = await window.supabaseClient.from("user_profiles").insert(updatedProfile)
          ...
          const { error } = await window.supabaseClient.from("user_profiles").update(updatedProfile).eq("id", user.id)
