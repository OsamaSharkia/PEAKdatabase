document.addEventListener("DOMContentLoaded", () => {
      ...
      if (resourceTableBody) {
        ...
        if (data && data.length > 0) {
          data.forEach((resource) => {
            ...
            // Format date
            const createdAt = new Date(resource.created_at).toLocaleDateString("en-US", {
              year: "numeric",
              month: "short",
              day: "numeric",
            })
            ...
            row.innerHTML = `
              ...
              <td>${createdAt}</td>
              <td>
                <button class="btn-small btn-edit-resource" data-id="${resource.id}">Edit</button>
                <button class="btn-small btn-delete-resource" data-id="${resource.id}">Delete</button>
              </td>
            `
          ...
          resourceTableBody.innerHTML = `<tr><td colspan="5" class="text-center">No resources found</td></tr>`
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (data) {
        ...
        editForm.addEventListener("submit", async (e) => {
          ...
          try {
            const { error } = await supabaseClient.from("user_profiles").update(updatedUser).eq("id", userId)
      ...
      // showNotification("Failed to get user details", "error")
    }
  }
  ...
  async function deleteUser(userId) {
    ...
    try {
      ...
      const { error: profileError } = await supabaseClient.from("user_profiles").delete().eq("id", userId)
  ...
  async function editResource(resourceId) {
    try {
      const { data, error } = await supabaseClient.from("resources").select("*").eq("id", resourceId).single()
      ...
      if (data) {
            ...
            <form id="edit-resource-form">
              ...
              <div class="form-group">
                ...
                <select id="edit-category">
                  ...
                  <option value="Funding" ${data.category === "Funding" ? "selected" : ""}>Funding</option>
                  <option value="University" ${data.category === "University" ? "selected" : ""}>University</option>
              ...
              <div class="form-group">
                ...
                <input type="url" id="edit-url" value="${data.url || ""}">
              </div>
              <button type="submit" class="btn btn-primary">Save Changes</button>
            </form>
          </div>
        ...
        editForm.addEventListener("submit", async (e) => {
          ...
          try {
            const { error } = await supabaseClient.from("resources").update(updatedResource).eq("id", resourceId)
      ...
      // showNotification("Failed to get resource details", "error")
    }
  }
  ...
  async function deleteResource(resourceId) {
    ...
    try {
      const { error } = await supabaseClient.from("resources").delete().eq("id", resourceId)
  ...
  // Add new resource form handling
  ...
  if (addResourceForm) {
    addResourceForm.addEventListener("submit", async (e) => {
      ...
      try {
        const { error } = await supabaseClient.from("resources").insert(newResource)
