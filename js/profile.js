document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Fix: Use window.supabase to avoid variable naming conflict
  ...
  setupForms(supabaseClient, session, profile)

  // Set up account deletion
  setupAccountDeletion(supabaseClient, session)

  // Load activity data
  loadActivityData(supabaseClient, session.user.id)

  // Load saved resources
  loadSavedResources(supabaseClient, session.user.id)
})
...
function setupAccountDeletion(supabase, session) {
  ...
  confirmDelete.addEventListener("click", async () => {
    try {
      ...
      // Note: This might require admin privileges, so we have a fallback
      try {
        await supabase.auth.admin.deleteUser(session.user.id)
      } catch (adminError) {
        console.error("Admin delete failed, trying sign out:", adminError)
...
async function loadActivityData(supabase, userId) {
  try {
    ...
    const { error: tableCheckError } = await supabase.from("user_activity").select("id").limit(1).single()
    ...
    if (tableCheckError) {
      ...
      emptyState.innerHTML = `
        ...
        <p>Activity tracking is not set up yet</p>
      `
      return
    }
...
async function loadSavedResources(supabase, userId) {
  try {
    ...
    const { error: tableCheckError } = await supabase.from("saved_resources").select("id").limit(1).single()
    ...
    if (tableCheckError) {
      ...
      emptyState.innerHTML = `
        ...
        <p>Saved resources feature is not set up yet</p>
